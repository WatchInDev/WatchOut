...
export default function EventsPage() {
  ...
  const parseEventImages = (imageField?: string | string[] | null): string[] => {
    ...
    if (typeof imageField === "string") return imageField.split(",").map(i => i.trim()).filter(Boolean);
    return [];
  };
  ...
  const handleDeleteComment = (comment: Comment, eventId: number, banAuthorId?: number) => {
    setConfirm({
      text: banAuthorId
      ...
      action: async () => {
        ...
        await queryClient.invalidateQueries({ queryKey: ["eventComments", eventId] });
        setConfirm(null);
      }
    });
  };
  ...
  const columns = useMemo<MRT_ColumnDef<Event>[]>(() => [
    ...
    },
    {
      ...
      Cell: ({ row }) => {
        ...
        if (!event.isActive) return <span className="text-sm opacity-70">Zakończone</span>;
        return (
          <div className="flex gap-2">
            <Button size="sm" variant="destructive" onClick={() => handleDeleteEvent(event.id)}>Usuń</Button>
            <Button size="sm" variant="destructive" onClick={() => handleDeleteEvent(event.id, event.authorId)}>Usuń + Zablokuj</Button>
          </div>
        );
      }
    }
  ...
  return (
    <div className="p-6">
      <Card className="p-4">
        ...
        <MaterialReactTable
          ...
          renderDetailPanel={({ row }) => {
            ...
            if (commentsQuery.isLoading) return <p className="p-4 text-sm opacity-70">Ładowanie komentarzy...</p>;
                    ...
                    <div className="space-y-2">
                      {comments.map((c) => (
                        <Card key={c.id} className="p-3 flex justify-between">
                          ...
                          <div className="flex gap-2">
                            <Button size="sm" variant="destructive" onClick={() => handleDeleteComment(c, event.id)}>Usuń</Button>
                            <Button size="sm" variant="destructive" onClick={() => handleDeleteComment(c, event.id, c.authorId)}>Usuń + Zablokuj</Button>
                          </div>
                        </Card>
                      ))}
                    </div>
      ...
      <Dialog open={!!confirm} onOpenChange={() => setConfirm(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Potwierdź akcję</DialogTitle></DialogHeader>
          <p>{confirm?.text}</p>
          <DialogFooter>
      ...
      <Dialog open={!!fullImage} onOpenChange={() => setFullImage(null)}>
        <DialogContent className="flex justify-center items-center">
          <img src={fullImage ?? ""} alt="" className="max-h-[80vh] max-w-full object-contain" />
        </DialogContent>
      </Dialog>
    </div>
  );
